...
export default function SalesEntry() {
    ...
    const [state, setState] = useState({
        ...
        currentUser: null // New field for user role
    });
    ...
    const { allSales, customerSearchInput, selectedPrintedCustomer, selectedUnprintedCustomer, editingSaleId,
        ...
        items, suppliers, isPrinting, billSize, gridPricePerKg, selectedSaleForBreakdown, currentUser } = state;
    ...
    const fetchInitialData = async () => {
        try {
            // Assuming user data is stored in localStorage or retrieved via an auth endpoint
    ...
    const formatReceiptValue = (val) => {
        ...
        if (roundedNum === Math.floor(roundedNum)) return Math.floor(roundedNum).toString();
...
<div style="margin-bottom:5px; border-bottom:1px solid #000;">
    ...
    <tbody>${itemsHtml}<tr style="border-top:1px solid #000;"><td colspan="3" style="text-align:left; padding:6px 4px; font-size:${fontSizeItems}; font-weight:bold;">${totalPacksSum}</td><td style="text-align:right; padding:6px 4px; font-size:${fontSizeItems}; font-weight:bold;"><div style="display:flex; flex-direction:column; align-items:flex-end;"><div style="font-size:0.9em;"></div></div></td><td style="text-align:right; padding:6px 4px; font-size:${fontSizeItems}; font-weight:bold;"><span style="display:inline-block; transform: translateX(-10px);">${formattedTotalSalesExcludingPackDue}</span></td></tr></tbody></table>
        ...
        <Layout style={{ backgroundColor: '#99ff99' }} billSize={billSize} handleBillSizeChange={handleBillSizeChange}>
            <div className="sales-layout" style={{ maxWidth: '1400px', margin: '0 auto' }}>
                ...
                <div className="three-column-layout" style={{ opacity: isLoading ? 0.7 : 1, display: 'grid', gridTemplateColumns: '200px 1fr 200px', gap: '16px', padding: '10px', marginTop: '-149px' }}>
                    ...
                    {/* --- CENTER COLUMN: Conditional Logic for Admin vs Pos User --- */}
                    <div className="center-form flex flex-col" style={{ backgroundColor: '#111439ff', padding: '20px', borderRadius: '0.75rem', color: 'white', height: '150.5vh', boxSizing: 'border-box', gridColumnStart: 2, gridColumnEnd: 3 }}>
                        {currentUser?.role === 'Admin' ? (
                            <div className="admin-farmer-view h-full flex flex-col gap-4">
                                ...
                                {/* This container divides the space into two vertical columns side-by-side */}
                            ...
                            /* ORIGINAL SALES FORM FOR NON-ADMINS */
                                    ...
                                    <form onSubmit={handleSubmit} className="space-y-4">
                                        ...
                                        <div className="w-full" style={{ display: "grid", gridTemplateColumns: "repeat(12, 1fr)", columnGap: "8px", alignItems: "end", marginTop: "8px" }}>
                                            ...
                                            {[{ id: 'weight', placeholder: "බර", fieldRef: refs.weight }, { id: 'price_per_kg_grid_item', placeholder: "මිල", fieldRef: refs.price_per_kg_grid_item }, { id: 'packs', placeholder: "අසුරුම්", fieldRef: refs.packs }, { id: 'total', placeholder: "TOTAL", fieldRef: refs.total, isReadOnly: true }].map(({ id, placeholder, fieldRef, isReadOnly = false }) => (
                                                <div key={id} style={{ ...(id === 'weight' && { gridColumnStart: 8, gridColumnEnd: 9, marginLeft: "-70px", width: "100px" }), ...(id === 'price_per_kg_grid_item' && { gridColumnStart: 9, gridColumnEnd: 10, marginLeft: "-30px", width: "100px" }), ...(id === 'packs' && { gridColumnStart: 10, gridColumnEnd: 11 }), ...(id === 'total' && { gridColumnStart: 11, gridColumnEnd: 14, marginLeft: "10px" }) }}>
