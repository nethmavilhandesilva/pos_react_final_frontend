              ...
              return (
                <li key={type === "printed" ? `${item.customerCode}-${item.billNo}` : item.customerCode} className="flex">
                  <button
                    ...
                    // 🟢 Light Blue BG/Black Text FIX using inline style for high specificity
                  ...
                  >
                    <span
                      // 🟢 Enforce Black Text on the span content
...
const ItemSummary = ({ sales, formatDecimal }) => {
 ...
 return (
  <div className="mt-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
    ...
    {/* Make this container horizontal scrollable */}
    <div className="flex gap-2 overflow-x-auto whitespace-nowrap">
      {Object.entries(summary).map(([itemName, data]) => (
        <div
          key={itemName}
          className="px-3 py-1 bg-white border border-gray-300 rounded-full text-xs font-medium inline-block"
        >
          <span className="font-semibold text-black">{itemName}:</span>
          <span className="ml-1 text-black">{data.totalWeight}kg</span>
          <span className="mx-1 text-black">/</span>
          <span className="text-black">{data.totalPacks}p</span>
        </div>
      ))}
    </div>
  </div>
...
export default function SalesEntry() {
  ...
  <button
    ...
    className={`px-4 py-1 text-sm text-white font-bold rounded-xl shadow transition ${
  ...
  {/* Your Given Amount Field (Placed to Right of F10 Button) */}
  <input
  id="given_amount"
  ...
  className="px-4 py-2 border rounded-xl text-right w-40 ml-6"  // ← space before input
/>
