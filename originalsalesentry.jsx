...
export default function SalesEntry() {
    ...
    const displayedSales = useMemo(() => {
        ...
        // Refs + recentSubmittedSales keep rows glued so they never blink out during rapid entry.
        const candidates = [];
        ...
        (sidebarSales || []).forEach(pushCandidate);
        (allSales || []).forEach(pushCandidate);

        const byId = new Map();
        candidates.forEach((sale) => {
            if (!sale?.id || isDeletedSaleId(deletedSaleIdsRef.current, sale.id)) return;
            const key = String(sale.id);
            const recent = recentSubmittedSalesRef.current.get(key)
                || recentSubmittedSalesRef.current.get(sale.id);
            // Just-updated bulk prices / submits always win in the table.
            if (recent?.sale && (Date.now() - recent.at) < RECENT_SALE_TRUST_MS) {
                byId.set(key, recent.sale);
                return;
            }
            if (!byId.has(key)) {
                byId.set(key, sale);
            ...
            const prevPrinted = String(prev?.bill_printed ?? '').trim().toUpperCase() === 'Y';
            const nextPrinted = String(sale?.bill_printed ?? '').trim().toUpperCase() === 'Y';
            if ((!prev?.bill_no && sale?.bill_no) || (!prevPrinted && nextPrinted)) {
                byId.set(key, sale);
            }
        });

        // Collapse tmp-* + confirmed pairs so one physical entry never shows twice,
        // and the confirmed row always wins once the POST lands.
        tempToRealIdRef.current.forEach((realId, tempId) => {
            const tKey = String(tempId);
            const rKey = String(realId);
            if (byId.has(tKey) && byId.has(rKey)) {
                byId.delete(tKey);
            } else if (byId.has(tKey) && !byId.has(rKey)) {
                // Real id not in pool yet — keep the temp painted (never blank the line).
                const tempSale = byId.get(tKey);
                const recentReal = recentSubmittedSalesRef.current.get(rKey)?.sale;
                if (recentReal) byId.set(rKey, recentReal);
                else if (tempSale) byId.set(tKey, tempSale);
            }
        });
        ...
        const rows = Array.from(byId.values()).sort((a, b) => {
            ...
            // Reliable timestamp extraction that works for both temp and real records
            const getSortTime = (sale) => {
                // For optimistic/temp records, use their creation time from the ID
                if (sale._optimistic || String(sale.id || '').startsWith('tmp-')) {
                    const idStr = String(sale.id);
                    const timestamp = parseInt(idStr.split('-')[1]);
                    if (!isNaN(timestamp)) return timestamp;
                }

                // For regular records, try various timestamp fields
                const timestamps = [
                    sale.created_at,
                    sale.updated_at,
                    sale.timestamp,
                ...
                // Final fallback: use the ID as a timestamp if it's numeric
        ...
        return rows;
    }, [allSales, sidebarSales, localTableSales, selectedUnprintedCustomer, selectedPrintedCustomer, formData.customer_code, isManualClear]);
    ...
    const refreshSidebarSales = useCallback(async (force = false) => {
        if (!isMountedRef.current) return;
        const now = Date.now();
        if (!force && now - lastSidebarRefreshAtRef.current < 1200) return;
        if (sidebarRefreshInFlightRef.current) {
            if (!force) return;
            // Allow a forced refresh to abort the in-flight one and restart.
    ...
    useEffect(() => {
        const interval = setManagedInterval(() => {
            ...
            // Keep sidebars tied to DB even during rapid entry.
            refreshSidebarSales(true);
        }, POS_WATCHDOG_MS);
    ...
    const fetchInitialData = useCallback(async (attempt = 0) => {
        ...
        try {
            ...
            lastSidebarSignatureRef.current = buildSalesSignature(sidebarNormalized);
    ...
    // Store current item data and price
    ...
    // Paint EVERY matching row instantly in ONE flushSync
    if (bulkPrice > 0 && bulkCustomer && bulkItem) {
        ...
        flushSync(() => {
            setState((prev) => {
                ...
                lastSidebarSignatureRef.current = buildSalesSignature(nextSidebar);
    ...
    // ⚡ INSTANT DOM FOCUS SHIFT (Zero-delay before async network call)
    ...
    // Ensure React state keeps fields preserved without wiping
    setFormData(prev => ({
        ...prev,
    ...
    // Fire off submit in background without blocking cursor movement
    void handleSubmit(e, {}, { bypassSignatureThrottle: true, preserveItem: true });
    ...
    return;
            ...
            if (currentFieldName === "packs") {
                ...
                flushSync(() => {
                    setState((prev) => {
                        // Keep unprinted sidebar live during rapid entry (before GET catches up).
                        let nextSidebar = prev.sidebarSales || [];
                        if (!stayOnPrinted && !isTempOrOptimisticSale(instantSale)) {
                            nextSidebar = [instantSale, ...nextSidebar.filter((s) => !sameSaleId(s.id, instantSale.id))];
                        } else if (!stayOnPrinted) {
                            // Even for temps, ensure customer appears in unprinted list by
                            // keeping any existing backend rows and not wiping sidebar.
                            nextSidebar = prev.sidebarSales || [];
                        }
                        sidebarSalesRef.current = nextSidebar;
                        return {
                            ...prev,
    ...
    const handleDeleteRecord = async (saleId) => {
        ...
        // 1. Add to tombstones / deleted sets IMMEDIATELY (before any async work)
        deletedSaleIdsRef.current.add(saleId);
        deletedSaleIdsRef.current.add(idStr);
        if (typeof saleId === 'string' && /^\d+$/.test(saleId)) {
            deletedSaleIdsRef.current.add(Number(saleId));
        }
        ...
        flushSync(() => {
            setState((prev) => ({
                ...
                sidebarSales: (prev.sidebarSales || []).filter((sale) => !sameSaleId(sale.id, saleId)),
                localTableSales: (prev.localTableSales || []).filter((sale) => !sameSaleId(sale.id, saleId)),
            }));
        });
        allSalesRef.current = (allSalesRef.current || []).filter((sale) => !sameSaleId(sale.id, saleId));
        displayedSalesRef.current = (displayedSalesRef.current || []).filter((sale) => !sameSaleId(sale.id, saleId));
        sidebarSalesRef.current = (sidebarSalesRef.current || []).filter((sale) => !sameSaleId(sale.id, saleId));
        lastSidebarSignatureRef.current = buildSalesSignature(sidebarSalesRef.current);
        ...
        try {
            ...
            updatedSales.forEach((sale) => {
                ...
                if (tempId) {
                    ...
                    // Keep temp alias pointing at the confirmed sale so a concurrent
                    // refresh cannot drop the line while React state catches up.
                    recentSubmittedSalesRef.current.set(tKey, {
                        sale: { ...sale, id: sale.id, _optimistic: false },
                        at: trackAt,
                    });
                }
            ...
            if (isMountedRef.current) {
                setState(prev => {
                    ...
                    lastSidebarSignatureRef.current = buildSalesSignature(nextSidebarSales);
            ...
            // Keep printed/unprinted sidebars on live backend data immediately.
            ...
            if (isMountedRef.current && pendingSubmitsRef.current.size === 0) {
                ...
                // Keep sidebars on DB truth after a burst of rapid submits.
                setManagedTimeout(() => refreshSidebarSales(true), 0);
            }
        }
    };
        ...
        if (!isCurrentlySelected) {
            ...
            if (rowsForTable.length === 0) {
                rowsForTable = (sidebarSalesRef.current || []).filter((s) => {
                    ...
                    if (!isPrinted) return String(s.bill_printed || '').trim().toUpperCase() !== 'Y';
                    return true;
                });
            }
            ...
            flushSync(() => {
                setState((prev) => {
                    ...
                    const nextSidebar = Array.from(sidebarById.values());
                    allSalesRef.current = nextAll;
                    sidebarSalesRef.current = nextSidebar;
                    lastSidebarSignatureRef.current = buildSalesSignature(nextSidebar);
...
<div class="receipt-content">
    ...
    const clearUiAfterPrint = useStableCallback(() => {
        tableCustomerScopeRef.current = '';
        editingSaleIdRef.current = null;
        selectedPrintedCustomerRef.current = null;
        selectedUnprintedCustomerRef.current = null;
        ...
        if (prefetch?.printAlreadyTriggered) {
            ...
            const finalizeUiAfterPrint = (finalBillNo) => {
                ...
                printedIds.forEach((idStr) => {
                    ...
                    upsertLocalTableSale(printedSale);
                });
                setState((prev) => {
                    const nextAll = prev.allSales.map(markPrintedRow);
                    const nextLocal = (prev.localTableSales || []).map(markPrintedRow);
                ...
                printAwaitingBillRef.current = false;
                printStartedAtRef.current = 0;
                // Clear form AFTER print dialog is up (next tick — no flushSync).
                setManagedTimeout(() => clearUiAfterPrint(), 0);
                setManagedTimeout(() => {
                    refreshSidebarSales(true);
                    refreshSalesData(true);
                }, 300);
    ...
    const handleShortcut = useStableCallback((e) => {
        ...
        if (e.key === "F1") {
            ...
            // Never soft-lock F1. Expire hung markPrinted waits so all-day use cannot stick.
            if (printAwaitingBillRef.current) {
                if (Date.now() - printStartedAtRef.current < PRINT_AWAIT_MAX_MS) return;
                printAwaitingBillRef.current = false;
                printInFlightRef.current = false;
                printStartedAtRef.current = 0;
            }
            ...
            // Abort any in-flight sales poll so it cannot compete with print.
            ...
            // Start from currently displayed table rows whenever present.
            ...
            // Prefer confirmed rows, but if the table only has just-saved temps keep them
            // mapped from recentSubmittedSales when a real id already exists.
            const confirmed = salesDataToValidate.filter(
                (s) => s?.id && !isTempOrOptimisticSale(s) && !isDeletedSaleId(deletedSaleIdsRef.current, s.id)
            );
            if (confirmed.length > 0) {
                salesDataToValidate = confirmed;
            } else {
                // Resolve temps -> latest confirmed copies from recent cache / pool when possible.
                const resolved = [];
                salesDataToValidate.forEach((s) => {
                    if (!s?.id || isDeletedSaleId(deletedSaleIdsRef.current, s.id)) return;
                    if (!isTempOrOptimisticSale(s)) {
                        resolved.push(s);
                        return;
                    }
                    // Keep temp only if it still has printable fields — usually means save is mid-flight.
                    const recentMatch = Array.from(recentSubmittedSalesRef.current.values())
                        .map((e) => e?.sale)
                        .find((r) => r && !isTempOrOptimisticSale(r)
                            && String(r.customer_code || '').toUpperCase() === String(s.customer_code || '').toUpperCase()
                            && String(r.item_code || '') === String(s.item_code || '')
                            && parseFloat(r.weight) === parseFloat(s.weight));
                    if (recentMatch) resolved.push(recentMatch);
                });
                salesDataToValidate = resolved.filter(
                    (s) => s?.id && !isTempOrOptimisticSale(s) && !isDeletedSaleId(deletedSaleIdsRef.current, s.id)
                );
            }

            if (salesDataToValidate.length === 0) {
                // Table looked populated with only unsaved temps.
                const tableHasRows = (displayedSales && displayedSales.length > 0)
                    || ((displayedSalesRef.current || []).length > 0);
                alert(tableHasRows
                    ? "වාර්තා සුරැකෙමින් පවතී. කරුණාකර නැවත F1 ඔබන්න."
                    : "මුද්‍රණය කිරීමට දත්ත නොමැත!");
                return;
            }
            ...
            // FAST PATH: bill already known -> open print dialog in this keydown turn.
            if (billNo) {
                printInFlightRef.current = true;
                printStartedAtRef.current = Date.now();
                const receiptHtml = buildFullReceiptHTML(
                ...
                const printedOk = writeAndPrintBill(buildPrintDocumentShell('Print Bill', receiptHtml));
                setTimeout(unlockPrintSoon, printedOk ? 350 : 0);
            ...
            // First print: assign bill_no, then open ONLY the system print dialog (hidden iframe).
            const markPrintedPromise = api.post(routes.markPrinted, {
            ...
            (async () => {
                try {
                    ...
                    setTimeout(unlockPrintSoon, 350);
                    handlePrintAndClear(null, {
                        salesToProcess: salesDataToValidate,
                        billNo: newBillNo,
    ...
    useEffect(() => {
        // Capture phase so F1 wins even if an input/select is focused.
                                                            ...
                                                            <Select
                                                                ...
                                                                onKeyDown={(e) => {
                                                                    ...
                                                                    if (e.key === "Enter" && isPlusTyped) {
                                                                        ...
                                                                        const tableRows = (displayedSalesRef.current && displayedSalesRef.current.length > 0)
                                                                        ...
                                                                        if (topRow?.item_code) {
                                                                            ...
                                                                            const remembered = lastEnteredItemRef.current;
                                                                            if (
                                                                                ...
                                                                                (allSalesRef.current || []).forEach(consider);
